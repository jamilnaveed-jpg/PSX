...
# ── Requests session with retry + browser headers ─────────────────────────────
def make_session():
...
def get_symbols():
    resp = SESSION.get(f"{BASE_URL}/api/symbols", timeout=20)
    data = resp.json()
    raw  = data.get("data", data) if isinstance(data, dict) else data
    syms = [str(s).strip() for s in raw if s]
    print(f"[SYMBOLS] {len(syms)} symbols fetched")
    return syms
...
def get_sector_map(symbols):
    ...
    for i, sym in enumerate(symbols):
        try:
            r = SESSION.get(f"{BASE_URL}/api/companies/{sym}", timeout=12)
        ...
        if (i+1) % 50  == 0:
            ...
            time.sleep(0.1)
    found = sum(1 for v in sm.values() if v != "—")
    print(f"[SECTORS] Complete — {found}/{len(symbols)} sectors found")
    return sm


def get_tick(symbol):
    r = SESSION.get(f"{BASE_URL}/api/ticks/REG/{symbol}", timeout=15)
    if r.status_code != 200:
        return None
...
def fetch_all_stocks(symbols, sm):
    ...
    print(f"[TICKS] Fetching {total} ticks...")
    for i, sym in enumerate(symbols):
        try:
            t = get_tick(sym)
            if t:
...
def main():
    ...
    print(f"\n{'='*62}\n  PSX Daily Scanner v9  —  {date_str}\n{'='*62}\n")

    symbols = get_symbols()
    if not symbols:
        print("[MAIN] No symbols returned — exiting cleanly.")
        sys.exit(0)
    ...
    if not stocks:
        print("[MAIN] ⚠️  No ticks fetched — market is likely closed (holiday) or API unavailable.")
        print("[MAIN] Exiting cleanly (exit code 0) — workflow will not be marked as failed.")
        sys.exit(0)
